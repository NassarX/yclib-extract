...
Usage:
    python scripts/fetch_pg_essays.py [--force] [--output-dir PATH]
...
def main():
    parser = argparse.ArgumentParser(description="Fetch Paul Graham essays from paulgraham.com")
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Custom output directory for essays (env: PG_ESSAYS_DIR)",
    )
    ...
    if args.output_dir:
        os.environ["PG_ESSAYS_DIR"] = args.output_dir

    # Create orchestrator and fetch essays
    orch = PipelineOrchestrator()

    print(f"🚀 Fetching Paul Graham essays...")
    print(f"   Output directory: {orch.pg_essays_dir}")
    print(f"   Mode: {'force refresh' if args.force else 'append-only'}")
    print()

    try:
        stats = orch.fetch_pg_essays(force=args.force)
